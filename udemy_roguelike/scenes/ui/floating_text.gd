...
func start(text: String):
	...
	tween.set_parallel()
	tween.tween_property(self, "global_position", self.global_position + (Vector2.UP * 16), .3)\
		.set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CUBIC)
	tween.chain()
		
	tween.tween_property(self, "global_position", self.global_position + (Vector2.UP * 48), .5)\
